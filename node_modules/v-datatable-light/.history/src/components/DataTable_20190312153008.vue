<template>
  <table :class="['v-datatable-light', css.table, tableHeight ? 'header-fixed' : '']">
    <thead :class="{}">
    ...
    <tbody :class="css.tbody" :style="tableHeight ? {height: tableHeight} : {} ">
      ...
      <template v-else-if="data.length">
        <tr v-for="(item, index) in data" :key="index" :class="css.tbodyTr">
          <td v-for="key in headers" :key="`${index}-${key.name}`" :class="css.tbodyTd" :style="getColumnWidth(key)">
            <slot
...
<style>
.v-datatable-light.header-fixed thead{
  display: block;
}
.v-datatable-light.header-fixed tbody{
  display: block;
  overflow-y: auto;
}
/* . */
...
export default {
  ...
  methods: {
    ...
    getColumnWidth: function (item) {
      ...
      console.log('RESULT', { width: `${item.width || this.defaultColumnWidth}px` });
      return { width: `${item.width || this.defaultColumnWidth}px` }
    }
  }
};
</script>
